...
function IconCreditCard({ className }: { className?: string }) {
  return <svg className={className} width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary-dark)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>;
}
    ...
    <div className="max-w-4xl mx-auto px-4">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div className="flex items-center gap-2">
          <IconCreditCard />
          <h1 className="text-xl font-bold text-[var(--foreground)]">收款码设置</h1>
        </div>
      </div>

      <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
        <div className="rounded-xl p-4 mb-6 text-sm" style={{"background":'var(--background)',"border":'1px solid rgba(201,168,124,0.3)'}}>
          <div className="flex items-start gap-2 text-[var(--foreground)]">
            <svg className="mt-0.5 flex-shrink-0" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--primary-dark)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
        ...
        {/* 商家名称 */}
        <div className="mb-8">
          <label className="block text-[var(--foreground)] font-bold mb-2">商家名称</label>
          <input
            type="text"
            value={settings.merchantName}
            onChange={e => setSettings(s => ({ ...s, merchantName: e.target.value }))}
            className="w-full px-4 py-3 border rgba(201,168,124,0.3) rounded-xl focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
            placeholder="显示在收款页面"
          />
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{"background":'rgba(74,117,86,0.1)'}}>
            ...
            <label className="text-[var(--foreground)] font-bold text-lg">微信收款码</label>
          </div>
          <div className="border-2 border-dashed rgba(201,168,124,0.2) rounded-2xl p-8 text-center" style={{"background":'var(--background)'}}>
            {settings.wechatQr ? (
              ...
              <div>
                <div className="w-12 h-12 mx-auto mb-4 rounded-full flex items-center justify-center" style={{"background":'rgba(74,117,86,0.1)'}}>
                ...
                <p className="text-[var(--foreground-muted)] mb-4">点击下方按钮上传微信收款码</p>
              </div>
            )}
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <label className="text-[var(--foreground)] font-bold text-lg">支付宝收款码</label>
          </div>
          <div className="border-2 border-dashed rgba(201,168,124,0.2) rounded-2xl p-8 text-center" style={{"background":'var(--background)'}}>
            {settings.alipayQr ? (
              <div className="relative inline-block">
              ...
              <div>
                <div className="w-12 h-12 mx-auto mb-4 rounded-full flex items-center justify-center" style={{"background":'rgba(59,130,246,0.1)'}}>
                ...
                <p className="text-[var(--foreground-muted)] mb-4">点击下方按钮上传支付宝收款码</p>
              </div>
            )}
        ...
        <div className="flex items-center gap-4 pt-4 border-t border-[var(--background-secondary)]">
          <button onClick={handleSave} disabled={saving}
            ...
            style={{"background":'var(--primary-dark)',"boxShadow":'0 4px 15px rgba(201,168,124,0.3)'}}>
            <IconSave />
            {saving ? '保存中...' : '保存设置'}
          </button>
          {saved && (
            <div className="flex items-center gap-2 text-[var(--sage)] font-bold">
              <IconCheck className="text-[var(--sage)]" />
              保存成功
            </div>
          )}
        </div>
      ...
      {(settings.wechatQr || settings.alipayQr) && (
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <div className="flex items-center gap-2 mb-6">
            <IconEye />
            <h2 className="text-xl font-bold text-[var(--foreground)]">客户看到的效果预览</h2>
          </div>
