  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-xl font-bold var(--foreground)">产品管理</h1>
          <p className="var(--foreground-muted) mt-1">管理商品 · 上传图片 · 更新库存</p>
        </div>
      ...
      <div className="flex gap-3 mb-8">
        <button onClick={() => setTab('list')} className={`px-5 py-2 rounded-full font-medium transition ${tab==='list'?'bg-[#c9a87c] text-white shadow':'bg-white border var(--foreground) hover:var(--background-card)'}`}>商品列表</button>
        <button onClick={() => { openAdd(); setTab('add'); }} className={`px-5 py-2 rounded-full font-medium transition ${tab==='add'?'bg-[#c9a87c] text-white shadow':'bg-white border var(--foreground) hover:var(--background-card)'}`}>添加/编辑</button>
      </div>

      {/* 添加/编辑表单 */}
      {tab === 'add' && (
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
          <h2 className="text-xl font-bold var(--foreground) mb-6">{editing ? '编辑商品' : '添加新商品'}</h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>商品名称 *</label>
              ...
              <div>
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>分类 *</label>
              ...
              <div>
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>价格（元） *</label>
              ...
              <div>
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>库存数量 *</label>
                <input type="number" min="0" value={form.stock||''} onChange={e=>setForm(f=>({...f,stock:parseInt(e.target.value)||0}))}
                  className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-[#c9a87c]" required />
              </div>
              <div className="md:col-span-2">
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>商品描述</label>
                <textarea rows={3} value={form.description} onChange={e=>setForm(f=>({...f,description:e.target.value}))}
                  placeholder="详细描述商品特点..." className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-[#c9a87c]" />
              </div>
              {/* 图片上传 */}
              <div className="md:col-span-2">
                <label className="block font-medium var(--foreground) mb-2" style={{fontSize:'1rem'}}>商品图片（上传到云存储）</label>
                <div className="flex items-start gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                      className="px-5 py-2.5 var(--background-secondary) var(--foreground) rounded-lg font-medium hover:var(--background-secondary) transition border">
                ...
                {/* 颜色选择（无图片时作为占位背景） */}
                <div className="mt-4">
                  <label className="block text-sm var(--foreground-muted) mb-2">背景色（无图片时显示）</label>
            ...
            <div className="flex gap-4 pt-4 border-t">
              <button type="button" onClick={() => setTab('list')} className="px-6 py-3 border rounded-xl font-bold var(--foreground) hover:var(--background-card)">取消</button>
        ...
        <>
          ...
          <div className="bg-white rounded-2xl shadow overflow-hidden">
            {loading ? (
              ...
              <div className="p-20 text-center var(--foreground-muted)"><div className="text-xl mb-4"></div><p className="text-xl">暂无商品，点击右上角添加</p></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="var(--background-card) border-b">
                    <tr>
                      <th className="text-left py-4 px-6 font-semibold var(--foreground)">商品</th>
                      <th className="text-left py-4 px-6 font-semibold var(--foreground)">分类</th>
                      <th className="text-left py-4 px-6 font-semibold var(--foreground)">价格</th>
                      <th className="text-left py-4 px-6 font-semibold var(--foreground)">库存</th>
                      <th className="text-left py-4 px-6 font-semibold var(--foreground)">操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {products.map(p => (
                      <tr key={p.id} className="border-t hover:var(--background-card) transition">
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-4">
                            <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${p.imageColor} flex items-center justify-center overflow-hidden flex-shrink-0`}>
                              {p.imageUrl ? <img src={p.imageUrl} alt={p.name} className="w-full h-full object-cover" /> : <span className="text-xl var(--foreground-light)">*</span>}
                            </div>
                            <div>
                              <div className="font-bold var(--foreground)">{p.name}</div>
                              <div className="text-sm var(--foreground-muted) max-w-xs truncate">{p.description}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6"><span className="px-3 py-1 var(--background-secondary) var(--foreground) rounded-full text-sm">{p.category}</span></td>
