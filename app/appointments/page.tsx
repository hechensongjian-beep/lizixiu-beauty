...
export default function AppointmentsPage() {
  ...
  const [appointments, setAppointments] = useState<Appointment[]>([]);
    ...
    useEffect(() => { document.title = '鍦ㄧ嚎棰勭害 - 涓藉Э绉€'; }, []);
  ...
  const getSelectedServiceNames = () => selectedServices.map(id => services.find(s => s.id === id)?.name).filter(Boolean).join('銆?);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await createAppointment({
        ...
        notes: notes + (selectedServices.length > 1 ? ` | 椤圭洰: ${getSelectedServiceNames()}` : ''),
        status: 'pending'
      });
      if (result.success) {
        setBooked(true);
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        <div className="bg-white rounded-xl shadow-md p-8" style={{border:'1px solid rgba(201,168,124,0.2)'}}>
          ...
          <h1 className="text-xl font-bold mb-2" style={{fontFamily:"'Noto Serif SC',serif",color:'#2a2a28'}}>棰勭害鎴愬姛</h1>
          <p className="mb-8" style={{color:'#6b6b68'}}>鎴戜滑灏嗗敖蹇‘璁ゆ偍鐨勯绾︼紝璇蜂繚鎸佹墜鏈虹晠閫?/p>
          <div className="rounded-xl p-5 mb-8 text-left" style={{background:'#faf8f5',border:'1px solid rgba(201,168,124,0.15)'}}>
            <div className="flex justify-between py-3" style={{borderBottom:'1px solid rgba(201,168,124,0.1)'}}>
              <span style={{color:'#6b6b68'}}>鏈嶅姟椤圭洰</span>
              <span className="font-bold text-right" style={{color:'#2a2a28'}}>{getSelectedServiceNames()}</span>
            </div>
            <div className="flex justify-between py-3" style={{borderBottom:'1px solid rgba(201,168,124,0.1)'}}>
              <span style={{color:'#6b6b68'}}>棰勮鏃堕暱</span>
              <span className="font-bold" style={{color:'#2a2a28'}}>绾getTotalDuration()}鍒嗛挓</span>
            </div>
            <div className="flex justify-between py-3" style={{borderBottom:'1px solid rgba(201,168,124,0.1)'}}>
              <span style={{color:'#6b6b68'}}>鏃ユ湡</span>
              <span className="font-bold" style={{color:'#2a2a28'}}>{date}</span>
            </div>
            <div className="flex justify-between py-3" style={{borderBottom:'1px solid rgba(201,168,124,0.1)'}}>
              <span style={{color:'#6b6b68'}}>鏃堕棿</span>
              <span className="font-bold" style={{color:'#2a2a28'}}>{time}</span>
            </div>
            <div className="flex justify-between py-3">
              <span style={{color:'#6b6b68'}}>缇庡甯?/span>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
      <div className="text-center mb-10">
        ...
        <h1 className="text-2xl font-bold mb-2" style={{fontFamily:"'Noto Serif SC',serif",color:'#2a2a28'}}>鍦ㄧ嚎棰勭害</h1>
        <p style={{color:'#6b6b68'}}>閫夋嫨鏈嶅姟銆佹椂闂村拰缇庡甯堬紝蹇€熷畬鎴愰绾?/p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-xl p-5" style={{boxShadow:'0 1px 4px rgba(0,0,0,0.03)',border:'1px solid rgba(201,168,124,0.15)'}}>
          <h2 className="text-xl font-bold mb-5" style={{fontFamily:"'Noto Serif SC',serif",color:'#2a2a28'}}>閫夋嫨鏈嶅姟椤圭洰</h2>
            ...
            {services.map(svc => (
              <button key={svc.id} type="button" onClick={() => toggleService(svc.id)}
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <span className="text-sm" style={{color:'#9b9b98'}}>{svc.duration || 60}鍒嗛挓</span>
                </div>
              </button>
            ))}
          </div>
          {selectedServices.length > 0 && (
            <div className="mt-4 p-4 rounded-xl flex justify-between items-center" style={{background:'#faf8f5',border:'1px solid rgba(201,168,124,0.15)'}}>
              <div>
                ...
                <div className="text-sm" style={{color:'#6b6b68'}}>绾getTotalDuration()}鍒嗛挓</div>
              </div>
              <div className="text-right">
                <div className="font-bold" style={{color:'#a88a5c'}}>{fmt(getTotalPrice())}</div>
                <div className="text-xs" style={{color:'#9b9b98'}}>{selectedServices.length}涓」鐩?/div>
              </div>
            </div>
          )}
        </div>
        ...
        {/* 鏃ユ湡+鏃堕棿+鍛樺伐 */}
        <div className="bg-white rounded-xl p-5" style={{boxShadow:'0 1px 4px rgba(0,0,0,0.03)',border:'1px solid rgba(201,168,124,0.15)'}}>
          <h2 className="text-xl font-bold mb-5" style={{fontFamily:"'Noto Serif SC',serif",color:'#2a2a28'}}>閫夋嫨鏃堕棿涓庣編瀹瑰笀</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
            <div>
              <label className="block font-medium mb-2" style={{color:'#2a2a28',fontSize:'1rem'}}>棰勭害鏃ユ湡</label>
            ...
            <div>
              <label className="block font-medium mb-2" style={{color:'#2a2a28',fontSize:'1rem'}}>鏈嶅姟缇庡甯?/label>
              <select value={staffId} onChange={e => {setStaffId(e.target.value);setTime('');}}
                ...
                onBlur={e=>e.target.style.borderColor='#e8e4df'}
                required>
                {staff.map(s => <option key={s.id} value={s.id}>{s.name} {s.role ? ` 路 ${s.role}` : ''}</option>)}
              </select>
            </div>
          </div>
          {date && (
            <div>
              <label className="block text-sm font-medium mb-2" style={{color:'#2a2a28'}}>
                閫夋嫨鏃堕棿
                {unavailable.length > 0 && <span className="text-xs ml-2" style={{color:'#9b9b98'}}>锛堢伆鑹蹭负宸查绾︼級</span>}
              </label>
        ...
        {/* 鑱旂郴淇℃伅 */}
        <div className="bg-white rounded-xl p-5" style={{boxShadow:'0 1px 4px rgba(0,0,0,0.03)',border:'1px solid rgba(201,168,124,0.15)'}}>
          <h2 className="text-lg font-bold mb-4" style={{fontFamily:"'Noto Serif SC',serif",color:'#2a2a28'}}>鑱旂郴淇℃伅</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-2" style={{color:'#2a2a28'}}>濮撳悕</label>
              <input type="text" value={name} onChange={e=>setName(e.target.value)} placeholder="鎮ㄧ殑濮撳悕"
                className="w-full px-3 py-2.5 rounded-lg outline-none transition"
            ...
            <div>
              <label className="block text-sm font-medium mb-2" style={{color:'#2a2a28'}}>鎵嬫満鍙?/label>
              <input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} placeholder="11浣嶆墜鏈哄彿"
                className="w-full px-3 py-2.5 rounded-lg outline-none transition"
                style={{border:'1.5px solid #e8e4df',background:'white'}}
                onFocus={e=>e.target.style.borderColor='#c9a87c'}
                onBlur={e=>e.target.style.borderColor='#e8e4df'}
            ...
            <div className="md:col-span-2">
              <label className="block text-sm font-medium mb-2" style={{color:'#2a2a28'}}>澶囨敞</label>
              <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={2} placeholder="濡傛湁鐗规畩闇€姹傝娉ㄦ槑"
        ...
        <button type="submit" disabled={submitting || !date || !time || selectedServices.length === 0}
          ...
          {submitting ? '鎻愪氦涓?..' : `纭棰勭害${selectedServices.length > 0 ? ' 路 ' + fmt(getTotalPrice()) : ''}`}
        </button>
      </form>

      <div className="text-center mt-6">
        <Link href="/" className="text-sm transition" style={{color:'#6b6b68'}}>
          <span className="mr-1">鈫?/span>杩斿洖棣栭〉
        </Link>
      </div>
    </div>
  );
