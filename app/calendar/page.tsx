            ...
            <div key={time} className="flex items-stretch min-h-[52px]">
              ...
              <div className="flex-1 py-2 px-3">
                {appt ? (
                  ...
                  <div className="h-8 flex items-center text-sm text-[var(--foreground-light)]">空闲</div>
                )}
              </div>
            </div>
          );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-xl font-bold text-[var(--foreground)]">预约日历</h1>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl shadow overflow-hidden">
            ...
            <div className="grid grid-cols-7 text-[var(--background-card)] border-b">
              {['日','一','二','三','四','五','六'].map(d => (
                <div key={d} className="py-2 text-center text-sm font-semibold text-[var(--foreground-muted)]">{d}</div>
              ))}
            </div>
            <div className="grid grid-cols-7">
              {[...Array(firstDay).fill(null), ...[...Array(daysInMonth)].map((_, i) => i+1)].map((day, idx) => {
                if (!day) return <div key={'empty-'+idx} className="min-h-[70px] text-[var(--background-card)] border-r border-b border-[var(--background-secondary)]"></div>;
